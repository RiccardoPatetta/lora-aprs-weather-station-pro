...
String buildAPRS() {
  ...
  sprintf(buffer,
  "%s-%d>APLHM0:!%02d%05.2f%c/%03d%05.2f%c_000/000g000t%03d"
  "h%02db%05d IAQ=%.0f CO2=%.0f VOC=%.2f GAS=%.0f VBAT=%.2fV",
...
void loop() {
  ...
  // Spegne display dopo 30 sec dal boot
if (bootScreenActive && millis() - displayStartTime > 30000) {
  displaySleep();
  bootScreenActive = false;
}

// Spegne display 20 sec dopo accensione APRS
if (!bootScreenActive && displayOn &&
    millis() - displayStartTime > 20000) {
  displaySleep();
}
